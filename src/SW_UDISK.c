...
void CMD_RD_WR_Deal_Pre( void )
{
    ...
    //printf("Data length to be transfered: %d\r\n", UDISK_Transfer_DataLen);
...
void UDISK_Out_EP_Deal( uint8_t *pbuf, uint16_t packlen )
{
    ...
    //printf("UDISK_Out_EP_Deal %d packlen\r\n", packlen);
    if( Udisk_Transfer_Status & DEF_UDISK_BLUCK_DOWN_FLAG )
    {
...
{    
    ...
    //printf("read %d,%d,%d\r\n", UDISK_Cur_Sec_Lba, DEF_UDISK_SECTOR_SIZE, UDISK_Pack_Size);
    vkart_read_data((UDISK_Cur_Sec_Lba * DEF_UDISK_SECTOR_SIZE + (UDISK_Sec_Pack_Count * UDISK_Pack_Size))>>1,  (uint16_t*)UDisk_Pack_Buffer, UDISK_Pack_Size>>1);
    pbuf = UDisk_Pack_Buffer;
    ...
    if( UDISK_Sec_Pack_Count == ( DEF_UDISK_SECTOR_SIZE / UDISK_Pack_Size ) )
    {
        ...
        UDISK_Cur_Sec_Lba++;
#endif
#if (STORAGE_MEDIUM == MEDIUM_VKART_FLASH)
	UDISK_Sec_Pack_Count = 0x00;
	UDISK_Cur_Sec_Lba++;
#endif
    }
...
void UDISK_Down_OnePack( uint8_t *pbuf, uint16_t packlen )
{
    ...
    //printf("udisk down onepack %d packlen, pack count = %d\r\n", packlen, UDISK_Sec_Pack_Count);
    if( UDISK_Sec_Pack_Count == 0x00 )
    {
...
#endif
	//printf("pack count = 0, address = 0x%08lx sec start addr %ld\r\n", address, sec_start_addr);
    }
    memcpy(UDisk_Down_Buffer + (uint32_t)UDISK_Sec_Pack_Count * UDISK_Pack_Size ,pbuf, UDISK_Pack_Size);
    //printf("memcpy from %p, into %p, len %d\r\n", UDisk_Down_Buffer + (uint32_t)UDISK_Sec_Pack_Count * UDISK_Pack_Size, pbuf, UDISK_Pack_Size);
    UDISK_Sec_Pack_Count++;
    UDISK_Transfer_DataLen -= UDISK_Pack_Size;
...
#elif (STORAGE_MEDIUM == MEDIUM_VKART_FLASH)
	vkart_write_block((uint16_t*) UDisk_Down_Buffer, sec_start_addr >> 1, DEF_FLASH_SECTOR_SIZE >> 1);
	//printf("flash %08lx size %d\r\n", sec_start_addr >> 1, DEF_FLASH_SECTOR_SIZE >> 1);
#endif
        if( UDISK_Transfer_DataLen == 0x00 )
        {
		//printf("out of the write block, UDISK_Transfer_DataLen == 0\r\n");
            Udisk_Transfer_Status &= ~DEF_UDISK_BLUCK_DOWN_FLAG;
            UDISK_Up_CSW( );
        }
	//printf("out of the write block, UDISK_Transfer_DataLen == %ld\r\n", UDISK_Transfer_DataLen);
        UDISK_Sec_Pack_Count = 0x00;
        UDISK_Cur_Sec_Lba++;
    }
}
